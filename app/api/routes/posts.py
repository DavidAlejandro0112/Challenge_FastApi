...
@router.get("/deleted/", response_model=list[Post])
async def read_deleted_posts(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    posts = await crud_post.get_deleted_posts(db, skip=skip, limit=limit)
    return posts
